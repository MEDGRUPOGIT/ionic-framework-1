...
import { AnimationBuilder, CssClassMap, RouterDirection, StyleEventDetail } from '../../../interface';
...
export class MedItem implements ComponentInterface, AnchorInterface, ButtonInterface {
  ...
  private delegateFocus(ev: Event, input: HTMLIonInputElement | HTMLIonTextareaElement) {
    const clickedItem = (ev.target as HTMLElement).tagName === 'MED-ITEM';
    ...
    if (clickedItem && firstActive) {
      (input as any).fireFocusEvents = false;
      (input as any).setBlur();
      input.setFocus();
      raf(() => {
        (input as any).fireFocusEvents = true;
      });
    }
  }
