...
export class TpAccordionGroup implements ComponentInterface {
  @Element() el!: HTMLTpAccordionGroupElement;
   ...
   * the value property.
   */
  @Event() ionChange!: EventEmitter<TpAccordionGroupChangeEventDetail>;
   ...
   * @internal
   */
  @Event() ionValueChange!: EventEmitter<TpAccordionGroupChangeEventDetail>;
...
`,
  ...
  async onKeydown(ev: KeyboardEvent) {
     ...
     * in ion-textarea.
     */
    const activeAccordionHeader = activeElement.closest(
    ...
    const accordionEl =
      activeElement.tagName === "ION-ACCORDION"
        ? activeElement
    ...
    let accordion: HTMLTpAccordionElement | undefined;
    if (ev.key === "ArrowDown") {
  ...
  private findNextAccordion(
    accordions: HTMLTpAccordionElement[],
  ...
  async getAccordions() {
    return Array.from(
      this.el.querySelectorAll(":scope > ion-accordion")
    ) as HTMLTpAccordionElement[];
  }
